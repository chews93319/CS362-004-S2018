...
//set RNDCARD3 to 0 to remove printfs from output
#define RNDCARD3 1
#define STCRND3  1
...
/*
 * Random Test Generator for Dominion Card: smithy
 */
...
void randtest3(void){
    ...
    do {
        ...
        extraCoins = 0;
        for (i=1;i>3;i++){
            if((G.deck[activePlayer][G.deckCount[activePlayer]-i]==copper) ||
                    (G.deck[activePlayer][G.deckCount[activePlayer]-i]==silver) ||
                    (G.deck[activePlayer][G.deckCount[activePlayer]-i]==gold)){
                extraCoins += G.deck[activePlayer][G.deckCount[activePlayer]-i] - 3;
            }
        }
     ...
     */
        ...
        //playedCard is correct card
        sprintf(msg, "Test Played Card is %s", TESTCARD);
        failqty += assertEQ(msg, testG.playedCards[0], G.hand[activePlayer][handpos]);
        
        //handCount + newCard - played (orig + 3 - 1)
