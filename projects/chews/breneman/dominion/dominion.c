...
int adventurerEffect(struct gameState *state, int currentPlayer, int handPos, int temphand[]) {
  ...
  while(drawntreasure < 2) {
    ...
    cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer] - 1];
      ...
      // this should just remove the most recently drawn card
...
int smithyEffect(struct gameState *state, int currentPlayer, int handPos) {
  ...
  // INTENTIONAL BUG: pass an invalid player
  for (i = 0; i < 3; i++) {
    drawCard(currentPlayer + 50, state);
  }
...
int villageEffect(struct gameState *state, int currentPlayer, int handPos) {
  // INTENTIONAL BUG: don't draw a card
  // drawCard(currentPlayer, state);
